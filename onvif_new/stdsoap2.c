...
void rose_write_file(const char* filename, const char* data, int size)
{
    if (data == NULL || size <= 0) {
        return;
    }
...
fsend(struct soap *soap, const char *s, size_t n)
{
  int nwritten, err;
  SOAP_SOCKET sk;
  // rose_write_file("c:/ddksample/soap-req.xml", s, n);
  soap->errnum = 0;
...
#endif
  c = soap_getchar(soap);
  // rose_write_file("c:/ddksample/soap-resp.xml", soap->buf, soap->buflen);
#ifdef WITH_GZIP
  if (c == 0x1F)
  {
